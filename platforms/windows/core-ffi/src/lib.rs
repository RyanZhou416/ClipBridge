...
mod ffi_tests {
    ...
    fn ffi_plan_text_over_soft_needs_confirm_then_force() {
        unsafe {
            ...
            let snap1 = CString::new(format!(r#"{{
              "type":"ClipboardSnapshot",
              "ts_ms":{ts},
              "kind":"text",
              "share_mode":"default",
            ...
            let v1: serde_json::Value = serde_json::from_str(&take_json(cb_plan_local_ingest(h, snap1.as_ptr()))).unwrap();
            assert!(v1["ok"].as_bool().unwrap());
            assert_eq!(v1["data"]["plan"]["needs_user_confirm"].as_bool().unwrap(), true);

            let snap2 = CString::new(format!(r#"{{
              "type":"ClipboardSnapshot",
              "ts_ms":{ts},
              "kind":"text",
              "share_mode":"force",
            ...
            let v2: serde_json::Value = serde_json::from_str(&take_json(cb_plan_local_ingest(h, snap2.as_ptr()))).unwrap();
    ...
    fn ffi_plan_file_list_ok() {
        unsafe {
            ...
            let v: serde_json::Value = serde_json::from_str(&take_json(cb_plan_local_ingest(h, snap.as_ptr()))).unwrap();
            assert!(v["ok"].as_bool().unwrap());
    ...
    fn ffi_ingest_text_returns_meta() {
        unsafe {
            ...
            let v: serde_json::Value = serde_json::from_str(&take_json(cb_ingest_local_copy(h, snap.as_ptr()))).unwrap();
            assert!(v["ok"].as_bool().unwrap());
