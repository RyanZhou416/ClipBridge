...
#### 4.2.3.1 默认软限制（用于“默认是否同步/是否自动预取”）
...
| file_list（按总大小） | 200 MB | 仅广播元数据；**粘贴/用户显式拉取时** 才传输文件内容 | 同上 |

说明：
- “自动预取”只针对 **text 且 size ≤ text_auto_prefetch_bytes**；图片/文件默认不做预取（避免无意消耗带宽/磁盘）。
