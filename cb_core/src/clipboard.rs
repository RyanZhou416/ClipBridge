...
use crate::policy::{decide, MetaStrategy, PolicyOutcome, SizeLimits};
	...
	match snap {
		...
		ClipboardSnapshot::FileList { files, ts_ms } => {
			...
			ItemMeta {
				...
				preview: ItemPreview {
					file_count: Some(metas.len() as u32),
					..Default::default()
				},
				content: ItemContent {
					mime: FILELIST_MIME.to_string(),
					sha256: sha,
					total_bytes: manifest.len() as i64,
				},
				files: metas,
				expires_ts_ms: Some(*ts_ms + 7 * 24 * 3600 * 1000),
			}
