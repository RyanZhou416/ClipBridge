use super::*;
use crate::api::{AppConfig, CoreConfig, CoreEventSink};
use crate::cas::Cas;
...
impl TestSink {
	...
	fn assert_no_event(&self, event_type: &str) {
		let events = self.events.lock().unwrap();
		assert!(
			events.iter().all(|e| e["type"] != event_type),
			"Should NOT receive {event_type}"
		);
	}
}
...
fn setup(name: &str, password: &str) -> TestContext {
	...
	let log_store = Arc::new(Mutex::new(
		LogStore::open(&config.data_dir).expect("Failed to init LogStore"),
	));
	let sink = Arc::new(TestSink::new());
	// 端口传 0 让系统自动分配，避免端口冲突
	let transport = Arc::new(
		Transport::new(0, &config.data_dir, &config.device_id, &config.account_uid).unwrap(),
	);
...
async fn link_peers(server: &TestContext, client: &TestContext) -> (Connection, Connection) {
	...
	let (client_res, server_res) =
		tokio::join!(client.transport.connect(&addr), server.transport.accept());
...
async fn test_handshake_success() {
	let srv_ctx = setup("srv_ok", "tag_secret");
	let cli_ctx = setup("cli_ok", "tag_secret");
	...
	let srv_evt = srv_ctx
		.sink
		.wait_for_event("PEER_ONLINE", Duration::from_secs(2))
		.await;
...
async fn test_auth_fail_tag_mismatch() {
	// 设置不同的 Account Tag
	let srv_ctx = setup("srv_diff", "tag_A");
	let cli_ctx = setup("cli_diff", "tag_B");
...
async fn test_tofu_reject_changed_fingerprint() {
	let srv_ctx = setup("srv_hack", "tag_same");
	let cli_ctx = setup("cli_victim", "tag_same");
