...
fn default_gc_history() -> i64 {
	50_000
}
fn default_gc_cas() -> i64 {
	1024 * 1024 * 1024
} // 1GB
...
pub fn decide(kind: ItemKind, size_bytes: i64, force: bool, limits: &SizeLimits) -> PolicyOutcome {
	...
	if size_bytes > hard {
		return PolicyOutcome::RejectedHardCap {
			code: "ITEM_TOO_LARGE",
		};
	}
	...
	if force {
		PolicyOutcome::Allowed {
			strategy: MetaStrategy::MetaOnlyLazy,
			needs_user_confirm: false,
		}
	} else {
		PolicyOutcome::Allowed {
